...
finaco_destination::finaco_destination( wxWindow* parent )
:
destination( parent )
{
	invalidInput = ",";
...
void finaco_destination::OnInitDialog( wxInitDialogEvent& event )
{
...
void finaco_destination::OnDeselected( wxListEvent& event )
{
// TODO: Implement OnDeselected
}

void finaco_destination::OnSelect( wxListEvent& event )
{
	SetCtrlState();
}

void finaco_destination::OnAdd( wxCommandEvent& event )
{
// save the old one
UpdateItem();
...
void finaco_destination::OnDelete( wxCommandEvent& event )
{
...
void finaco_destination::OnEcho( wxCommandEvent& event )
{
int sel = GetSelectedDestinationItem();
if (sel == -1)
return;
...
void finaco_destination::OnNameText( wxCommandEvent& event )
{
UpdateItem();
...
void finaco_destination::OnDestinationHostText( wxCommandEvent& event )
{
	UpdateItem();
}

void finaco_destination::OnDestinationPortText( wxCommandEvent& event )
{
	UpdateItem();
}

void finaco_destination::OnDestinationAETitleText( wxCommandEvent& event )
{
	UpdateItem();
}

void finaco_destination::OnOurAETitleText( wxCommandEvent& event )
{
	UpdateItem();
}

void finaco_destination::OnOK( wxCommandEvent& event )
{
	event.Skip();
}


int finaco_destination::GetSelectedDestinationItem()
{
	long sel = -1;
	return m_destinationList->GetNextItem(sel, wxLIST_NEXT_ALL, wxLIST_STATE_SELECTED);
}

void finaco_destination::UpdateItem(int sel)
{
...
void  finaco_destination::SetCtrlState()
{
