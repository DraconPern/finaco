...
engine::engine()
	: query(patientdata), mover(patientdata)
{
...
void engine::LoadGlobalDestinationList()
{
	...
	wxRegKey registry;
	registry.SetName(wxRegKey::HKLM, "Software\\Policies\\FrontMotion\\finaco\\Destinations");
...
void engine::StartQuery(int destination)
{
	...
	query.DoQueryAsync(dest);
}

void engine::StopQuery()
{
	query.Cancel();
}

void engine::StartMove(int destination, std::string moveae, int threads)
{
	// find the destination
	DestinationEntry dest;
	...
	mover.DoMoveAsync(dest, moveae, threads);
}

void engine::StopMove()
{
	mover.Cancel();
}