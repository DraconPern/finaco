...
{
	std::string thisversion = FINACO_VERSION;
	std::string version, mustupdate, message;
	finaco_updateCheck updatecheckdlg(NULL);
	...
	try
	{
		...
		version = pt.get_child("finaco.version").data();
		mustupdate = pt.get_child("finaco.mustupdate").data();
		message = pt.get_child("finaco.message").data();
	...
	if (version != thisversion)
	{
		...
		updatecheckdlg.m_updatetext = wxT("A new version of finaco is available:");
		updatecheckdlg.ShowModal();
	}
...
int getUpdateJSON(std::string &updateinfo)
{
	...
	try
	{
		...
		std::string checkurl = "/software/finaco/";
		...
		request_stream << "User-Agent: finaco/" << FINACO_VERSION;
